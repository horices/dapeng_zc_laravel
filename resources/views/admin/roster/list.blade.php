        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr title="{{ $roster->qq_nickname }}" style="@if($roster->is_old == 1) opacity:0.5; @endif">
                    ...
                    @if( $roster->is_old == 0)
                    <a class="link_3" href="{{ route("admin.roster.follow.add",['roster_id'=>$roster->id]) }}" title="点击添加">点击添加</a>
                    @endif
                    </td>
                    <td class="grade grade4 grade5 grade9 grade10">
                        ...
                        <a class="@if($roster->is_old != 1) ajaxLink @endif" method="post" showLoading="1" data="{id:{{$roster->id}}}" url="{{route('admin.roster.index.open-course')}}" @if($roster->roster_type == 2 && $roster->group_status != 2) style='display:none;' @endif title="开通">开通</a>
                            @if($userInfo->grade <= 5 )
                            <a class="@if($roster->is_old != 1) ajaxLink @endif" method="post" showLoading="1" warning="确认{{$roster->roster_no}}{{ $roster->roster_type_text}}号与手机号{{$roster->dapeng_user_mobile}}解绑吗？" data="{roster_id:{{$roster->id}}}" url="{{route('admin.roster.unbind')}}" title="解绑">解绑</a>
                            @endif
                        @else
                        ...
                        @endif
                        <a class="@if($roster->is_old != 1) ajaxLink @endif" data="{url:'{{$roster->reg_url_prama}}'}" wx="{{$roster->wx}}" qq="{{$roster->qq}}" url="{{route('admin.roster.index.set-reg-url')}}" callback="registerUrl" @if($roster->roster_type == 2 && $roster->group_status != 2) style='display:none;' @endif title="链接">链接</a>
                    </td>
                </tr>
                @endforeach
                <tr>
