...
class StatisticsController extends BaseController
{
    ...
    function getSeoerStatistics(){
        ...
        $result = $roster->select([
            'inviter_id','is_reg','course_type','group_status',DB::raw("count(*) as num")
        ])->groupBy(['inviter_id','is_reg','course_type','group_status'])
            //->where('addtime','>',1513339653)
            ->whereIn('inviter_id',$list->pluck('uid'))->get();
    ...
    function getAdviserStatistics(){
        ...
        $result = $roster->select([
            'last_adviser_id','is_reg','course_type','group_status',DB::raw("count(*) as num")
        ])->groupBy(['last_adviser_id','is_reg','course_type','group_status'])
            //->where('addtime','>',1513339653)
            ->whereIn('last_adviser_id',$list->pluck('uid'))->get();
        $statistics = $user_statistics = [];
        $statistics['user_total'] = 0; //所有数据总量
        $statistics['user_total_reg_num_0'] = 0;   //未注册总人数
        $statistics['user_total_reg_num_1'] = 0;   //已注册总人数
